...
# of "the_list".
'''Function containing the first and last of the list'''
def first_and_last(the_list):
    return [the_list[0],the_list[-1]] #square brackets for lists
...
def part_reverse(the_list, beginning, end): #user puts in their list and own perameters
    if end > beginning and beginning>=0 and end<= len(the_list):
        rev= the_list[beginning: (end + 1)] #colon means from 2nd to 3rd perameter and the +1 means it includes the latter perameter
        rev.reverse() #variable.reverse reverses the output list
        return rev
    else :
        raise ValueError

print(part_reverse([1,2,3,4,5,6], 1,4))
...
def repeat_at_index(the_list, index): #index is position of no in list
    the_list.insert
    return
